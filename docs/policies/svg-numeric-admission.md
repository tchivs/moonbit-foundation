...
| `SVG-NUM-RELATIVE` | Direct relative-path branches in `path_data` | Relative additions for the supported direct `m/l/h/v/c/q/a` command routes | Derived admission after every direct relative-coordinate arithmetic result, including cubic and quadratic control points. |
