...
function New-Phase08R8ProductionContext([string]$ExpectedRepository,[string]$ExpectedRemote){
  ...
  $rows=@((Invoke-R8PreLiveGit @('ls-remote','--tags',$ExpectedRemote)).lines);$base=New-Phase08R7ProductionContext $ExpectedRepository $ExpectedRemote -RemoteTagRows $rows
