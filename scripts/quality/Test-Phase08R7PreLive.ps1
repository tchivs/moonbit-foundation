...
try{
  ...
  foreach($tagRecord in @($policy.initial_attempt_family.terminal_negative_history)){
    ...
    if($resolved.peel_sha-cne$tagRecord.source_sha-or($tagRecord.attempt-ceq'r5'-and$resolved.tag_object_sha-cne$tagRecord.tag_object_sha)){throw 'P08-R7-REMOTE-TAG-POSITIVE: exact remote binding drifted.'}
  }
  ...
  $bad=Copy-R7Context $context;$bad.summaries.plan_08_16.ancestor_of_head=$false;Confirm-R7Failure 'P08-R7-SUMMARY' {Assert-Phase08R7PreLive $bad}
