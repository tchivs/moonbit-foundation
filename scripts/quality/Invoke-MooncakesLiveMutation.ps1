...
function Assert-LiveRequest {
  ...
  if ($Request.release_ref -cne 'refs/tags/modules-v0.1.0-r8' -or $Request.source_sha -cnotmatch '^[0-9a-f]{40}$' -or
      ...
      @($history | Where-Object { $_ -cnotmatch '^[0-9a-f]{64}$' }).Count -ne 0 -or (@($history | Select-Object -Unique)).Count -ne 8 -or
      ($history -join ',') -cne ($expectedHistory -join ',') -or $Request.historical_history_set_sha256 -cne $historySet -or
      $historySet -cne '48de74c184100e34651e9e050e6949b252414746d13f78e04855fd28b69c8580' -or
