...
function Assert-LiveRequest {
  ...
  if ($Request.release_ref -cne 'refs/tags/modules-v0.1.0-r7' -or $Request.source_sha -cnotmatch '^[0-9a-f]{40}$' -or
      ...
      @($history | Where-Object { $_ -cnotmatch '^[0-9a-f]{64}$' }).Count -ne 0 -or (@($history | Select-Object -Unique)).Count -ne 7 -or
      ($history -join ',') -cne ($expectedHistory -join ',') -or $Request.historical_history_set_sha256 -cne $historySet -or
      $historySet -cne '93523aa11f0ab84736d7fa3b1cb500ade23043a4d01a3e07d205400436900334' -or
