...
foreach($required in @('refs/tags/modules-v0.1.0-r9','R8HistoryPath','historical_r8_sha256','mnf-phase08-r9-handoff.json','Copy-P08CanonicalPreparedArchive')){
  if($hostedSource.IndexOf($required,[StringComparison]::Ordinal) -lt 0){throw "P08-R9-HOSTED-STATIC: missing '$required'."}
}
...
foreach ($required in @(
  ...
  'refs/tags/modules-v0.1.0-r9','historical_attempts_sha256:','historical_r8_sha256','historical_history_set_sha256','publish --frozen --dry-run','native_runtime_verified','whoami.stdout','whoami.stderr',
...
. $hostedPath -Mode PublisherDryRun -Repository tchivs/moonbit-foundation -Workflow publish-modules.yml -ReleaseRef refs/tags/modules-v0.1.0-r9 -SourceSha ('1'*40) -RootIntentSha256 ('a'*64) -IntentSha256 ('a'*64) -PreparedManifestSha256 ('b'*64) -TargetModule mb-core -LocatorPath (Join-Path ([IO.Path]::GetTempPath()) 'unused-locator.json') -ArtifactRoot (Join-Path ([IO.Path]::GetTempPath()) 'unused-root') -LibraryOnly
...
function Assert-P08HostedDispatchFields {
  ...
  $script:p08HostedFieldsGh={
    ...
    if($CommandArguments[0] -ceq 'run' -and $CommandArguments[1] -ceq 'list'){
      ...
      $run=[pscustomobject][ordered]@{
        databaseId=$script:p08HostedFieldsRunId;headBranch='modules-v0.1.0-r9';headSha=('1'*40);status='completed';conclusion='success'
    ...
    if($CommandArguments[0] -ceq 'run' -and $CommandArguments[1] -ceq 'view'){
      return ([pscustomobject][ordered]@{
        databaseId=$script:p08HostedFieldsRunId;attempt=1;headBranch='modules-v0.1.0-r9';headSha=('1'*40);event='workflow_dispatch'
  ...
  try{
    $null=Invoke-P08HostedDispatch -Operation $Operation -Repo 'tchivs/moonbit-foundation' -WorkflowPath 'publish-modules.yml' `
      -Ref 'refs/tags/modules-v0.1.0-r9' -Sha ('1'*40) -RootIntent ('a'*64) -CurrentIntent ('b'*64) -PreparedDigest ('c'*64) `
      -Module 'mb-core' -PriorId $PriorId -PriorArtifact $PriorArtifact -Packet $Packet -Receipt $Receipt `
      -AttemptZeroHistory $AttemptZeroHistory -R1History $R1History -R2History $R2History -R3History $R3History -R4History $R4History -R5History $R5History -R6History $R6History -R7History $R7History -R8History $R8History
  }finally{
  ...
  $expectedPrefix=@('workflow','run','publish-modules.yml','--repo','tchivs/moonbit-foundation','--ref','modules-v0.1.0-r9')
...
try{
  ...
  for($i=0;$i -lt 9;$i++){
    ...
    [IO.File]::WriteAllText(@($attemptZeroHistory,$r1History,$r2History,$r3History,$r4History,$r5History,$r6History,$r7History,$r8History)[$i],($projection|ConvertTo-Json -Depth 30 -Compress),[Text.UTF8Encoding]::new($false))
  }
  ...
  $commonFields=@(
    'release_ref=refs/tags/modules-v0.1.0-r9',('source_sha='+('1'*40)),('root_intent_sha256='+('a'*64)),('intent_sha256='+('b'*64)),
  ...
  Assert-P08HostedDispatchFields -Operation HostedPreflight -PriorId '' -PriorArtifact '' -Packet '' -Receipt '' -AttemptZeroHistory $attemptZeroHistory -R1History $r1History -R2History $r2History -R3History $r3History -R4History $r4History -R5History $r5History -R6History $r6History -R7History $r7History -R8History $r8History -ExpectedFields (@(
  ...
  Assert-P08HostedDispatchFields -Operation PublishOne -PriorId '9001' -PriorArtifact 'mnf-checkpoint-9001-1' -Packet $packet -Receipt $receipt -AttemptZeroHistory $attemptZeroHistory -R1History $r1History -R2History $r2History -R3History $r3History -R4History $r4History -R5History $r5History -R6History $r6History -R7History $r7History -R8History $r8History -ExpectedFields (@(
...
function New-R9HandoffFixture {
  ...
  $history=@($control.initial_attempt_family.terminal_negative_history)
  for($i=0;$i -lt 9;$i++){
    $name=@('attempt-zero','r1','r2','r3','r4','r5','r6','r7','r8')[$i];$paths[$name]=Join-Path $Root "$name.json"
    $projection=[ordered]@{};foreach($property in $history[$i].PSObject.Properties){if($property.Name -cne 'record_sha256'){$projection[$property.Name]=$property.Value}}
  ...
  if($Variant -ceq 'mutation_authorized'){
    ...
    $packet=[pscustomobject][ordered]@{schema_version='mnf-phase08-mutation-authorization-packet/1';release_ref='refs/tags/modules-v0.1.0-r9';boundary_sha=('1'*40);packet_sha256=''}
  ...
  $bindings=[pscustomobject][ordered]@{
    release_ref='refs/tags/modules-v0.1.0-r9';boundary_sha=('1'*40);execution_root=[IO.Path]::GetFullPath($Root)
    ...
    attempt_zero_history_path=[IO.Path]::GetFullPath($paths.'attempt-zero');r1_history_path=[IO.Path]::GetFullPath($paths.r1);r2_history_path=[IO.Path]::GetFullPath($paths.r2);r3_history_path=[IO.Path]::GetFullPath($paths.r3);r4_history_path=[IO.Path]::GetFullPath($paths.r4);r5_history_path=[IO.Path]::GetFullPath($paths.r5);r6_history_path=[IO.Path]::GetFullPath($paths.r6);r7_history_path=[IO.Path]::GetFullPath($paths.r7);r8_history_path=[IO.Path]::GetFullPath($paths.r8)
    historical_history_set_sha256='39e45ed9aecf1788d106a043dd4b421243a577b66534d0748ca61937a0de86a8'
