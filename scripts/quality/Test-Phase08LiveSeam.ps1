...
$productionHandoff=[IO.Path]::GetFullPath((Join-Path ([IO.Path]::GetTempPath()) 'mnf-phase08-r10-handoff.json'))
...
foreach($required in @('refs/tags/modules-v0.1.0-r10','R9HistoryPath','historical_r9_sha256','mnf-phase08-r10-handoff.json','Copy-P08CanonicalPreparedArchive','ValidatePreAuthorization')){
...
function New-LiveTestRequest {
  [pscustomobject][ordered]@{
    ...
    }; release_ref='refs/tags/modules-v0.1.0-r10'
...
foreach ($required in @(
  ...
  'refs/tags/modules-v0.1.0-r10','historical_attempts_sha256:','historical_r9_sha256','historical_history_set_sha256','publish --frozen --dry-run','native_runtime_verified','whoami.stdout','whoami.stderr',
...
. $hostedPath -Mode PublisherDryRun -Repository tchivs/moonbit-foundation -Workflow publish-modules.yml -ReleaseRef refs/tags/modules-v0.1.0-r10 -SourceSha ('1'*40) -RootIntentSha256 ('a'*64) -IntentSha256 ('a'*64) -PreparedManifestSha256 ('b'*64) -TargetModule mb-core -LocatorPath (Join-Path ([IO.Path]::GetTempPath()) 'unused-locator.json') -ArtifactRoot (Join-Path ([IO.Path]::GetTempPath()) 'unused-root') -LibraryOnly
...
function Assert-P08HostedDispatchFields {
  ...
  try{
    $null=Invoke-P08HostedDispatch -Operation $Operation -Repo 'tchivs/moonbit-foundation' -WorkflowPath 'publish-modules.yml' `
      ...
      -AttemptZeroHistory $AttemptZeroHistory -R1History $R1History -R2History $R2History -R3History $R3History -R4History $R4History -R5History $R5History -R6History $R6History -R7History $R7History -R8History $R8History -R9History $R9History
  }finally{
...
try{
  ...
  for($i=0;$i -lt 10;$i++){
    ...
    [IO.File]::WriteAllText(@($attemptZeroHistory,$r1History,$r2History,$r3History,$r4History,$r5History,$r6History,$r7History,$r8History,$r9History)[$i],($projection|ConvertTo-Json -Depth 30 -Compress),[Text.UTF8Encoding]::new($false))
  }
  ...
  Assert-P08HostedDispatchFields -Operation HostedPreflight -PriorId '' -PriorArtifact '' -Packet '' -Receipt '' -AttemptZeroHistory $attemptZeroHistory -R1History $r1History -R2History $r2History -R3History $r3History -R4History $r4History -R5History $r5History -R6History $r6History -R7History $r7History -R8History $r8History -R9History $r9History -ExpectedFields (@(
  ...
  Assert-P08HostedDispatchFields -Operation PublishOne -PriorId '9001' -PriorArtifact 'mnf-checkpoint-9001-1' -Packet $packet -Receipt $receipt -AttemptZeroHistory $attemptZeroHistory -R1History $r1History -R2History $r2History -R3History $r3History -R4History $r4History -R5History $r5History -R6History $r6History -R7History $r7History -R8History $r8History -R9History $r9History -ExpectedFields (@(
...
function New-R9HandoffFixture {
  ...
  $history=@($control.initial_attempt_family.terminal_negative_history)
  for($i=0;$i -lt 10;$i++){
    $name=@('attempt-zero','r1','r2','r3','r4','r5','r6','r7','r8','r9')[$i];$paths[$name]=Join-Path $Root "$name.json"
    $projection=[ordered]@{};foreach($property in $history[$i].PSObject.Properties){if($property.Name -cne 'record_sha256'){$projection[$property.Name]=$property.Value}}
  ...
  if($Variant -ceq 'mutation_authorized'){
    ...
    $packet=[pscustomobject][ordered]@{schema_version='mnf-phase08-mutation-authorization-packet/1';release_ref='refs/tags/modules-v0.1.0-r10';boundary_sha=('1'*40);packet_sha256=''}
  ...
  $bindings=[pscustomobject][ordered]@{
    release_ref='refs/tags/modules-v0.1.0-r10';boundary_sha=('1'*40);execution_root=[IO.Path]::GetFullPath($Root)
    ...
    attempt_zero_history_path=[IO.Path]::GetFullPath($paths.'attempt-zero');r1_history_path=[IO.Path]::GetFullPath($paths.r1);r2_history_path=[IO.Path]::GetFullPath($paths.r2);r3_history_path=[IO.Path]::GetFullPath($paths.r3);r4_history_path=[IO.Path]::GetFullPath($paths.r4);r5_history_path=[IO.Path]::GetFullPath($paths.r5);r6_history_path=[IO.Path]::GetFullPath($paths.r6);r7_history_path=[IO.Path]::GetFullPath($paths.r7);r8_history_path=[IO.Path]::GetFullPath($paths.r8);r9_history_path=[IO.Path]::GetFullPath($paths.r9)
    historical_history_set_sha256='ea679099fbb3201708368847e0530c024e08fa9da5fd9100391cab61f1a1e7ee'
