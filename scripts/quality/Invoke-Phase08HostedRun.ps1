...
function Write-P08AuthorizationReceipt {
  ...
  if($packet.schema_version -cne 'mnf-phase08-mutation-authorization-packet/1' -or $packet.release_ref -cne 'refs/tags/modules-v0.1.0-r4' -or
    ...
    Throw-P08HostedRule 'P08-RECEIPT-PACKET' 'Authorization packet is not the exact digest-bound r4 packet.'
  }
...
function Write-P08ActiveAttempt {
  ...
  $pathFields=@('boundary_locator_path','artifact_index_path','attempt_zero_history_path','r1_history_path','r2_history_path','r3_history_path')
  ...
  $value=[pscustomobject][ordered]@{
    schema_version='mnf-phase08-active-attempt/1';release_ref='refs/tags/modules-v0.1.0-r4';boundary_sha=[string]$Bindings.boundary_sha;execution_root=$root
...
function Open-P08ActiveAttempt([Parameter(Mandatory)][string]$Path) {
  ...
  if((@($value.PSObject.Properties.Name)-join ',')-cne($names-join ',')-or$value.schema_version-cne'mnf-phase08-active-attempt/1'-or$value.release_ref-cne'refs/tags/modules-v0.1.0-r4'){Throw-P08HostedRule 'P08-ACTIVE-CLOSED' 'Active attempt shape drifted.'}
  $history=Get-ReleaseInitialHistoryBinding
  if($value.attempt_zero_history_sha256-cne$history.historical_attempt_zero_sha256-or$value.r1_history_sha256-cne$history.historical_r1_sha256-or$value.r2_history_sha256-cne$history.historical_r2_sha256-or$value.r3_history_sha256-cne$history.historical_r3_sha256-or$value.historical_history_set_sha256-cne$history.historical_history_set_sha256){Throw-P08HostedRule 'P08-ACTIVE-DIGEST' 'Active attempt history family drifted.'}
...
function Write-P08HostedHandoff {
  ...
  $bindings=[ordered]@{
    ...
    r2_history_path=$active.r2_history_path;r2_history_sha256=$active.r2_history_sha256;r3_history_path=$active.r3_history_path;r3_history_sha256=$active.r3_history_sha256;historical_history_set_sha256=$active.historical_history_set_sha256
...
function New-P08PreparedAttempt {
  ...
  if($ReleaseRef -cne 'refs/tags/modules-v0.1.0-r4'){Throw-P08HostedRule 'P08-PREPARE-R4-BINDING' 'PrepareAttempt requires the exact r4 release ref.'}
  ...
  if($control.initial_attempt_family.current_attempt -cne 'r4' -or $history.Count -ne 4 -or ($history.attempt -join ',') -cne 'attempt_zero,r1,r2,r3' -or
      ($history.release_ref -join ',') -cne 'refs/tags/modules-v0.1.0,refs/tags/modules-v0.1.0-r1,refs/tags/modules-v0.1.0-r2,refs/tags/modules-v0.1.0-r3' -or
      @($history|Where-Object{$_.mutation_performed-ne$false-or$_.authority_acquired-ne$false}).Count -ne 0){
    Throw-P08HostedRule 'P08-PREPARE-HISTORY' 'The exact four-entry terminal-negative history is required.'
  }
  ...
  if($resolvedRef -cne [string]$Boundary.boundary_sha){Throw-P08HostedRule 'P08-PREPARE-REF' 'The r4 ref does not peel to the durable boundary.'}
  ...
  try {
    ...
    if($intentDigest -cnotmatch '^[0-9a-f]{64}$' -or (Get-P08Sha256 $intentPath) -cne $intentDigest -or
      ...
      Throw-P08HostedRule 'P08-PREPARE-INTENT-BINDING' 'Fresh r4 intent/root binding is invalid.'
    }
    ...
    $attemptZero=$historyProjections[0];$r1Negative=$historyProjections[1];$r2Negative=$historyProjections[2];$r3Negative=$historyProjections[3]
    ...
    $attemptZeroDigest=Get-P08Sha256 $attemptZeroPath;$r1HistoryDigest=Get-P08Sha256 $r1HistoryPath;$r2HistoryDigest=Get-P08Sha256 $r2HistoryPath;$r3HistoryDigest=Get-P08Sha256 $r3HistoryPath
    $historySet=([Convert]::ToHexString([Security.Cryptography.SHA256]::HashData([Text.UTF8Encoding]::new($false).GetBytes((@($attemptZeroDigest,$r1HistoryDigest,$r2HistoryDigest,$r3HistoryDigest)-join"`n"))))).ToLowerInvariant()
    ...
    $request=[pscustomobject][ordered]@{
      ...
      historical_attempt_zero_sha256=$attemptZeroDigest;historical_r1_sha256=$r1HistoryDigest;historical_r2_sha256=$r2HistoryDigest;historical_r3_sha256=$r3HistoryDigest;historical_history_set_sha256=$historySet
    }
    ...
    $genesisCommand=[pscustomobject][ordered]@{
      ...
      outcome='accepted';recorded_at_utc=$commitTime;run_identity=[pscustomobject][ordered]@{repository='tchivs/moonbit-foundation';run_id='1';artifact_name='publisher-genesis-r4';artifact_sequence=0}
    }
...
function Open-P08BoundaryStore {
  ...
  if ([IO.Path]::GetFullPath([string]$value.artifact_root) -cne [IO.Path]::GetFullPath($Artifacts) -or
      $value.repository -cne $Repository -or $value.workflow -cne $Workflow -or $value.release_ref -cne 'refs/tags/modules-v0.1.0-r4' -or
...
function Invoke-P08HostedDispatch {
  param([string]$Operation,[string]$Repo,[string]$WorkflowPath,[string]$Ref,[string]$Sha,[string]$RootIntent,[string]$CurrentIntent,[string]$PreparedDigest,[string]$Module,[string]$PriorId,[string]$PriorArtifact,[string]$Packet,[string]$Receipt,[string]$AttemptZeroHistory,[string]$R1History,[string]$R2History,[string]$R3History)
  $before=@{}; foreach($run in @(Get-P08Runs $Repo $WorkflowPath)){ $before[[string]$run.databaseId]=$true }
  if([string]::IsNullOrWhiteSpace($AttemptZeroHistory)-or[string]::IsNullOrWhiteSpace($R1History)-or[string]::IsNullOrWhiteSpace($R2History)-or[string]::IsNullOrWhiteSpace($R3History)){Throw-P08HostedRule 'P08-HOSTED-HISTORY' 'All four terminal-negative history files are required for dispatch.'}
  $history=Get-ReleaseInitialHistoryBinding
  $historyFiles=@($AttemptZeroHistory,$R1History,$R2History,$R3History);$historyDigests=@($historyFiles|ForEach-Object{Get-P08Sha256 $_})
  if(($historyDigests-join',')-cne(@($history.historical_attempt_zero_sha256,$history.historical_r1_sha256,$history.historical_r2_sha256,$history.historical_r3_sha256)-join',')){Throw-P08HostedRule 'P08-HOSTED-HISTORY' 'Individual terminal-negative history files drifted.'}
...
if ($ReleaseRef -cne 'refs/tags/modules-v0.1.0-r4' -or $SourceSha -cnotmatch '^[0-9a-f]{40}$' -or $RootIntentSha256 -cnotmatch '^[0-9a-f]{64}$' -or
    $IntentSha256 -cnotmatch '^[0-9a-f]{64}$' -or $PreparedManifestSha256 -cnotmatch '^[0-9a-f]{64}$') {
  Throw-P08HostedRule 'P08-HOSTED-R4-BINDING' 'Only the exact r4 release binding is accepted.'
}
...
switch ($Mode) {
  ...
  'PersistAuthorizationReceipt' {
    if($TargetModule -cne 'mb-core' -or [string]::IsNullOrWhiteSpace($MutationAuthorizationPacketPath) -or
       [string]::IsNullOrWhiteSpace($AttemptZeroHistoryPath) -or [string]::IsNullOrWhiteSpace($R1HistoryPath) -or [string]::IsNullOrWhiteSpace($R2HistoryPath) -or [string]::IsNullOrWhiteSpace($R3HistoryPath)){
      Throw-P08HostedRule 'P08-RECEIPT-BINDINGS' 'Core packet and all four terminal-negative histories are required.'
    }
    ...
    $bindings=[pscustomobject][ordered]@{
      ...
      attempt_zero_history_path=$AttemptZeroHistoryPath;r1_history_path=$R1HistoryPath;r2_history_path=$R2HistoryPath;r3_history_path=$R3HistoryPath;historical_history_set_sha256=(Get-ReleaseInitialHistoryBinding).historical_history_set_sha256;mutation_authorization_packet_path=$MutationAuthorizationPacketPath
  ...
  'WriteHandoff' {
    if([string]::IsNullOrWhiteSpace($AttemptZeroHistoryPath) -or [string]::IsNullOrWhiteSpace($R1HistoryPath) -or [string]::IsNullOrWhiteSpace($R2HistoryPath) -or [string]::IsNullOrWhiteSpace($R3HistoryPath)){Throw-P08HostedRule 'P08-HANDOFF-HISTORY' 'All four terminal-negative histories are required.'}
    $activeFull=Join-Path $ArtifactRoot 'active-attempt.json'
    $stamp=if($null-eq$CreatedAt){[DateTime]::UtcNow}else{$CreatedAt}
    if(-not(Test-Path -LiteralPath $activeFull -PathType Leaf)){
      ...
      $bindings=[pscustomobject][ordered]@{
        boundary_sha=$BoundarySha;execution_root=$ExecutionRoot;boundary_locator_path=$BoundaryLocatorPath;artifact_root=$ArtifactRoot;artifact_index_path=[string]$store.locator.index_path
        attempt_zero_history_path=$AttemptZeroHistoryPath;r1_history_path=$R1HistoryPath;r2_history_path=$R2HistoryPath;r3_history_path=$R3HistoryPath;historical_history_set_sha256=(Get-ReleaseInitialHistoryBinding).historical_history_set_sha256;mutation_authorization_packet_path=$null
    ...
    $fixed=[IO.Path]::GetFullPath((Join-Path ([IO.Path]::GetTempPath()) 'mnf-phase08-r4-handoff.json'))
...
$run=Invoke-P08HostedDispatch -Operation $Mode -Repo $Repository -WorkflowPath $Workflow -Ref $ReleaseRef -Sha $SourceSha -RootIntent $RootIntentSha256 -CurrentIntent $IntentSha256 -PreparedDigest $PreparedManifestSha256 -Module $TargetModule -PriorId $PriorRunId -PriorArtifact $PriorArtifactName -Packet $MutationAuthorizationPacketPath -Receipt $AuthorizationReceiptPath -AttemptZeroHistory $AttemptZeroHistoryPath -R1History $R1HistoryPath -R2History $R2HistoryPath -R3History $R3HistoryPath
