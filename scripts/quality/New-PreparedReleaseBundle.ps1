...
$expectedInitialHistory = [ordered]@{
  ...
  set = '93523aa11f0ab84736d7fa3b1cb500ade23043a4d01a3e07d205400436900334'
}
$inventory = @(
...
function Assert-PreparedBindings {
  ...
  if ($Repository -cne 'tchivs/moonbit-foundation' -or $Actor -cne 'tchivs' -or $RunId -cnotmatch '^[1-9][0-9]*$' -or
      $RunAttempt -lt 1 -or $ReleaseRef -cnotmatch '^refs/tags/modules-(v0[.]1[.]0-r7|correction-[1-9][0-9]*)$' -or
  ...
  $history = @($HistoricalAttemptZeroSha256,$HistoricalR1Sha256,$HistoricalR2Sha256,$HistoricalR3Sha256,$HistoricalR4Sha256,$HistoricalR5Sha256,$HistoricalR6Sha256)
  if (@($history | Where-Object { $_ -cnotmatch '^[0-9a-f]{64}$' }).Count -ne 0 -or @($history | Select-Object -Unique).Count -ne 7 -or
      $HistoricalHistorySetSha256 -cnotmatch '^[0-9a-f]{64}$') { Throw-PreparedRule 'PREP14-HISTORICAL-BINDING' 'Seven distinct historical-negative digests and their ordered set are required.' }
  ...
  if ($HistoricalHistorySetSha256 -cne $expectedSet -or $HistoricalAttemptZeroSha256 -cne $expectedInitialHistory.attempt_zero -or
      $HistoricalR1Sha256 -cne $expectedInitialHistory.r1 -or $HistoricalR2Sha256 -cne $expectedInitialHistory.r2 -or $HistoricalR3Sha256 -cne $expectedInitialHistory.r3 -or $HistoricalR4Sha256 -cne $expectedInitialHistory.r4 -or $HistoricalR5Sha256 -cne $expectedInitialHistory.r5 -or $HistoricalR6Sha256 -cne $expectedInitialHistory.r6 -or
...
function Assert-PreparedJournalBinding {
  ...
  try {
    Assert-PreparedClosedProperties 'request' $request @(
      ...
      'historical_attempt_zero_sha256','historical_r1_sha256','historical_r2_sha256','historical_r3_sha256','historical_r4_sha256','historical_r5_sha256','historical_r6_sha256','historical_history_set_sha256'
    )
  ...
  if ($ReleaseRef -ceq 'refs/tags/modules-v0.1.0-r7' -and
    ...
    Throw-PreparedRule 'PREP10-JOURNAL-BINDING' 'r7 must be a fresh initial root with no predecessor.'
  }
}
