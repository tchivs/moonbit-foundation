...
function Get-ReleaseInitialHistoryBinding {
  ...
  if ($history.Count -ne 6 -or ($history.attempt -join ',') -cne 'attempt_zero,r1,r2,r3,r4,r5') { Throw-ReleaseRule -Id 'REL04-HISTORY-BINDING' -Message 'canonical six-attempt history is missing or reordered.' }
  $digests = @($history.record_sha256)
  if (@($digests | Where-Object { -not (Test-ReleaseSha256Text $_) } | Select-Object -Unique).Count -ne 0 -or @($digests | Select-Object -Unique).Count -ne 6) { Throw-ReleaseRule -Id 'REL04-HISTORY-BINDING' -Message 'history record digests are missing or duplicated.' }
  ...
  return [pscustomobject][ordered]@{
    ...
    historical_r2_sha256=[string]$digests[2];historical_r3_sha256=[string]$digests[3];historical_r4_sha256=[string]$digests[4];historical_r5_sha256=[string]$digests[5];historical_history_set_sha256=$set
  }
}
...
function Get-ReleaseAuthorizationReceiptProjection {
  ...
  return [pscustomobject][ordered]@{
    ...
    historical_r1_sha256=[string]$Receipt.historical_r1_sha256;historical_r2_sha256=[string]$Receipt.historical_r2_sha256;historical_r3_sha256=[string]$Receipt.historical_r3_sha256;historical_r4_sha256=[string]$Receipt.historical_r4_sha256;historical_r5_sha256=[string]$Receipt.historical_r5_sha256
...
function New-ReleaseAuthorizationReceipt {
  ...
  $value=[pscustomobject][ordered]@{
    schema_version='mnf-phase08-authorization-receipt/1';release_ref='refs/tags/modules-v0.1.0-r6';boundary_sha=$BoundarySha;source_sha=$SourceSha
    ...
    historical_r2_sha256=$history.historical_r2_sha256;historical_r3_sha256=$history.historical_r3_sha256;historical_r4_sha256=$history.historical_r4_sha256;historical_r5_sha256=$history.historical_r5_sha256;historical_history_set_sha256=$history.historical_history_set_sha256
...
function Assert-ReleaseAuthorizationReceipt {
  ...
  $history=Get-ReleaseInitialHistoryBinding
  foreach($name in @('historical_attempt_zero_sha256','historical_r1_sha256','historical_r2_sha256','historical_r3_sha256','historical_r4_sha256','historical_r5_sha256','historical_history_set_sha256')){if([string]$Receipt.$name -cne [string]$history.$name){Throw-ReleaseRule -Id 'REL04-HISTORY-BINDING' -Message "receipt history binding drifted for $name."}}
...
function Assert-ReleasePhase08Handoff {
  ...
  if ($Handoff.schema_version -cne 'mnf-phase08-handoff/1' -or $Handoff.release_ref -cne 'refs/tags/modules-v0.1.0-r6' -or $Handoff.boundary_sha -cnotmatch '^[0-9a-f]{40}$') { Throw-ReleaseRule -Id 'REL04-HANDOFF-BINDING' -Message 'handoff identity drifted.' }
  ...
  $history=Get-ReleaseInitialHistoryBinding
  foreach($pair in @(@('attempt_zero_history_sha256','historical_attempt_zero_sha256'),@('r1_history_sha256','historical_r1_sha256'),@('r2_history_sha256','historical_r2_sha256'),@('r3_history_sha256','historical_r3_sha256'),@('r4_history_sha256','historical_r4_sha256'),@('r5_history_sha256','historical_r5_sha256'))){if([string]$Handoff.($pair[0]) -cne [string]$history.($pair[1])){Throw-ReleaseRule -Id 'REL04-HISTORY-BINDING' -Message "handoff history binding drifted for $($pair[0])."}}
...
function Assert-ReleaseIntentObject {
  ...
  if ($kind -ceq 'initial') {
    ...
    if (@($policy.initial_attempt_family.terminal_negative_history.source_sha) -ccontains [string]$Intent.source_sha) { Throw-ReleaseRule -Id 'REL01-HISTORICAL-SOURCE' -Message 'terminal attempt-zero, r1, r2, r3, r4, or r5 source cannot be current r6 authority.' }
  } else {
