...
function Get-ReleaseInitialHistoryBinding {
  ...
  if ($history.Count -ne 9 -or ($history.attempt -join ',') -cne 'attempt_zero,r1,r2,r3,r4,r5,r6,r7,r8') { Throw-ReleaseRule -Id 'REL04-HISTORY-BINDING' -Message 'canonical nine-attempt history is missing or reordered.' }
  $digests = @($history.record_sha256)
  if (@($digests | Where-Object { -not (Test-ReleaseSha256Text $_) } | Select-Object -Unique).Count -ne 0 -or @($digests | Select-Object -Unique).Count -ne 9) { Throw-ReleaseRule -Id 'REL04-HISTORY-BINDING' -Message 'history record digests are missing or duplicated.' }
  ...
  return [pscustomobject][ordered]@{
    ...
    historical_r2_sha256=[string]$digests[2];historical_r3_sha256=[string]$digests[3];historical_r4_sha256=[string]$digests[4];historical_r5_sha256=[string]$digests[5];historical_r6_sha256=[string]$digests[6];historical_r7_sha256=[string]$digests[7];historical_r8_sha256=[string]$digests[8];historical_history_set_sha256=$set
  }
}
...
function New-ReleaseAuthorizationReceipt {
  ...
  $value=[pscustomobject][ordered]@{
    schema_version='mnf-phase08-authorization-receipt/1';release_ref='refs/tags/modules-v0.1.0-r9';boundary_sha=$BoundarySha;source_sha=$SourceSha
...
function Assert-ReleaseAuthorizationReceipt {
  ...
  if ($Receipt.schema_version -cne 'mnf-phase08-authorization-receipt/1' -or $Receipt.release_ref -cne 'refs/tags/modules-v0.1.0-r9' -or
  ...
  $history=Get-ReleaseInitialHistoryBinding
  foreach($name in @('historical_attempt_zero_sha256','historical_r1_sha256','historical_r2_sha256','historical_r3_sha256','historical_r4_sha256','historical_r5_sha256','historical_r6_sha256','historical_r7_sha256','historical_r8_sha256','historical_history_set_sha256')){if([string]$Receipt.$name -cne [string]$history.$name){Throw-ReleaseRule -Id 'REL04-HISTORY-BINDING' -Message "receipt history binding drifted for $name."}}
...
function Assert-ReleasePhase08Handoff {
  ...
  if ($Handoff.schema_version -cne 'mnf-phase08-handoff/1' -or $Handoff.release_ref -cne 'refs/tags/modules-v0.1.0-r9' -or $Handoff.boundary_sha -cnotmatch '^[0-9a-f]{40}$') { Throw-ReleaseRule -Id 'REL04-HANDOFF-BINDING' -Message 'handoff identity drifted.' }
