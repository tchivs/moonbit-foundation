...
function Invoke-P08R13BoundaryGit([string]$Root,[string[]]$Arguments) {
...
if(-not(Test-Path -LiteralPath $clone -PathType Container)){Throw-P08R13Boundary 'P08-R13-CLONE' 'Clone root is missing.'}
$gitRoot=((Invoke-P08R13BoundaryGit $clone @('rev-parse','--show-toplevel'))-join '').Trim()
if([IO.Path]::GetFullPath($gitRoot) -cne $clone){Throw-P08R13Boundary 'P08-R13-CLONE' 'Clone root is not the exact Git worktree root.'}

$policyPath=Join-Path $clone 'policy/release-control.json'
if(-not(Test-Path -LiteralPath $policyPath -PathType Leaf)){Throw-P08R13Boundary 'P08-R13-POLICY' 'Clone-local release-control policy is missing.'}
...
if($canonicalReleaseRef -cnotmatch '^refs/tags/modules-v0[.]1[.]0-r[1-9][0-9]*$'){
  Throw-P08R13Boundary 'P08-R13-POLICY' 'Clone-local policy does not select one canonical immutable release tag.'
}
$tagObject=((Invoke-P08R13BoundaryGit $clone @('rev-parse','--verify',$canonicalReleaseRef))-join '').Trim()
$peeled=((Invoke-P08R13BoundaryGit $clone @('rev-parse','--verify',"$canonicalReleaseRef^{}"))-join '').Trim()
$head=((Invoke-P08R13BoundaryGit $clone @('rev-parse','HEAD'))-join '').Trim()
if($tagObject -cne $ExpectedTagObject -or $peeled -cne $ExpectedPeeledSourceSha -or $head -cne $ExpectedPeeledSourceSha){
  Throw-P08R13Boundary 'P08-R13-TAG' 'Clone policy ref, fetched tag object, peel, and detached HEAD do not match the immutable tag identity.'
}

$hosted=Join-Path $clone 'scripts/quality/Invoke-Phase08HostedRun.ps1'
if(-not(Test-Path -LiteralPath $hosted -PathType Leaf)){Throw-P08R13Boundary 'P08-R13-HOSTED' 'Clone is missing the HostedRun entrypoint.'}
