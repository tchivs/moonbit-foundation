...
function Assert-PublisherRequest {
  ...
  $history=@([string]$Request.historical_attempt_zero_sha256,[string]$Request.historical_r1_sha256,[string]$Request.historical_r2_sha256,[string]$Request.historical_r3_sha256,[string]$Request.historical_r4_sha256,[string]$Request.historical_r5_sha256)
  $expectedHistory=@(
  ...
  if (@($history | Where-Object { $_ -cnotmatch '^[0-9a-f]{64}$' }).Count -ne 0 -or (@($history | Select-Object -Unique)).Count -ne 6 -or
      ...
      $historySet -cne 'faed8af741e3e2f80d385fcc496649e12bf32889d6dff424fad3c8916acf9c0a') {
    Throw-PublisherRule 'PUB16-HISTORY' 'The exact six terminal-negative histories and their ordered set digest are required.'
  }
