...
function Assert-Phase08AttemptSchemas {
  ...
  foreach ($branch in @('mutationAuthorizationPacket','exactExistingAuthority','moduleAuthority')) {
    ...
    if ($schemaBranch.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r7') {
      throw "REL04-AUTHORITY-REF: $branch does not require r7."
    }
  ...
  if ($receipt.type -cne 'object' -or $receipt.additionalProperties -ne $false -or
      ...
      $receipt.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r7' -or
    ...
    throw 'REL04-RECEIPT-SCHEMA: authorization receipt is not the exact closed r7 contract.'
  }
  foreach ($entry in $historyFields.GetEnumerator()) {
  ...
  foreach ($branch in @($mutation,$exact)) {
    if ($branch.type -cne 'object' -or $branch.additionalProperties -ne $false -or
      $branch.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r7' -or
        $branch.properties.created_at_utc.'$ref' -cne '#/$defs/utc' -or
        $handoff.'$defs'.utc.pattern -cne '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$') {
      throw 'REL04-HANDOFF-SCHEMA: handoff branch is not closed, r7-bound, and UTC-canonical.'
    }
    ...
    foreach ($field in @('r2_history_path','r2_history_sha256','r3_history_path','r3_history_sha256','r4_history_path','r4_history_sha256','r5_history_path','r5_history_sha256','r6_history_path','r6_history_sha256')) {
  ...
  $receiptFixture = [pscustomobject][ordered]@{
    schema_version='mnf-phase08-authorization-receipt/1';release_ref='refs/tags/modules-v0.1.0-r7'
  ...
  if (-not (($receiptFixture | ConvertTo-Json -Depth 30 -Compress) | Test-Json -SchemaFile $receiptSchemaPath -ErrorAction Stop)) { throw 'REL04-RECEIPT-SCHEMA: valid r7 receipt fixture failed.' }
  foreach ($mutate in @(
    { param($x) $x.release_ref='refs/tags/modules-v0.1.0-r6' },
