...
function Assert-Phase08AttemptSchemas {
  ...
  foreach ($branch in @('mutationAuthorizationPacket','exactExistingAuthority','moduleAuthority')) {
    ...
    if ($schemaBranch.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r6') {
      throw "REL04-AUTHORITY-REF: $branch does not require r6."
    }
  ...
  $receiptFields = @('schema_version','release_ref','boundary_sha','source_sha','packet_sha256','historical_attempt_zero_sha256','historical_r1_sha256','historical_r2_sha256','historical_r3_sha256','historical_r4_sha256','historical_r5_sha256','historical_history_set_sha256','response','created_at_utc','receipt_sha256')
  if ($receipt.type -cne 'object' -or $receipt.additionalProperties -ne $false -or
      (@($receipt.required) -join ',') -cne ($receiptFields -join ',') -or
      $receipt.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r6' -or
    ...
    throw 'REL04-RECEIPT-SCHEMA: authorization receipt is not the exact closed r6 contract.'
  }
  foreach ($entry in $historyFields.GetEnumerator()) {
  ...
  foreach ($branch in @($mutation,$exact)) {
    if ($branch.type -cne 'object' -or $branch.additionalProperties -ne $false -or
      $branch.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r6' -or
        $branch.properties.created_at_utc.'$ref' -cne '#/$defs/utc' -or
        $handoff.'$defs'.utc.pattern -cne '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$') {
      throw 'REL04-HANDOFF-SCHEMA: handoff branch is not closed, r6-bound, and UTC-canonical.'
    }
    ...
    foreach ($field in @('r2_history_path','r2_history_sha256','r3_history_path','r3_history_sha256','r4_history_path','r4_history_sha256','r5_history_path','r5_history_sha256')) {
  ...
  $receiptFixture = [pscustomobject][ordered]@{
    schema_version='mnf-phase08-authorization-receipt/1';release_ref='refs/tags/modules-v0.1.0-r6'
  ...
  if (-not (($receiptFixture | ConvertTo-Json -Depth 30 -Compress) | Test-Json -SchemaFile $receiptSchemaPath -ErrorAction Stop)) { throw 'REL04-RECEIPT-SCHEMA: valid r6 receipt fixture failed.' }
  foreach ($mutate in @(
    { param($x) $x.release_ref='refs/tags/modules-v0.1.0-r5' },
...
function Assert-IntentContract {
  ...
  $history = @($policy.initial_attempt_family.terminal_negative_history)
  if ($history.Count -ne 6) { throw 'REL01-HISTORICAL-ATTEMPT: exact attempt-zero/r1/r2/r3/r4/r5 history is required.' }
  ...
  if (($history.attempt -join ',') -cne 'attempt_zero,r1,r2,r3,r4,r5') { throw 'REL01-HISTORY-ORDER: terminal history order drifted.' }
  ...
  if (@($history.record_sha256 | Select-Object -Unique).Count -ne 6) { throw 'REL01-HISTORY-DIGEST: terminal history digests are not distinct.' }
  ...
  if ($policy.initial_attempt_family.current_attempt -cne 'r6' -or
      $policy.initial_profile.release_ref -cne 'refs/tags/modules-v0.1.0-r6' -or $policy.initial_profile.correction_sequence -ne 0 -or
  ...
  if ($schema.'$defs'.initialIntent.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r6') {
    throw 'REL01-INITIAL-PROFILE: initial schema does not require r6.'
  }
  if ($preparedSchema.properties.release_ref.pattern -cne '^refs/tags/modules-(v0[.]1[.]0-r6|correction-[1-9][0-9]*)$') {
    throw 'REL01-INITIAL-PROFILE: prepared schema does not require r6 or a correction ref.'
  }
...
function Invoke-FocusedIntentTests {
  ...
  try {
    ...
    foreach ($oldRef in @('refs/tags/modules-v0.1.0','refs/tags/modules-v0.1.0-r1','refs/tags/modules-v0.1.0-r2','refs/tags/modules-v0.1.0-r3','refs/tags/modules-v0.1.0-r4','refs/tags/modules-v0.1.0-r5')) {
