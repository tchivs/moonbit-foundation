...
function Assert-Phase08AttemptSchemas {
  ...
  if ($history.Count -ne 13 -or ($history.attempt -join ',') -cne 'attempt_zero,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12') {
    throw 'REL04-HISTORY-ORDER: authority requires attempt-zero through r12 in canonical order.'
  }
  $historyFields = [ordered]@{
    ...
    historical_r12_sha256 = [string]$history[12].record_sha256
  ...
  foreach ($branch in @('mutationAuthorizationPacket','exactExistingAuthority','moduleAuthority')) {
    ...
    if ($schemaBranch.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r13') {
      throw "REL04-AUTHORITY-REF: $branch does not require r13."
    }
  ...
  if ($receipt.type -cne 'object' -or $receipt.additionalProperties -ne $false -or
    ...
    throw 'REL04-RECEIPT-SCHEMA: authorization receipt is not the exact closed r13 contract.'
  }
  foreach ($entry in $historyFields.GetEnumerator()) {
  ...
  foreach ($branch in @($mutation,$exact)) {
    if ($branch.type -cne 'object' -or $branch.additionalProperties -ne $false -or
      $branch.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r13' -or
    ...
    foreach ($entry in ([ordered]@{
      ...
      r12_history_sha256=$historyFields.historical_r12_sha256
  ...
  $receiptFixture = [pscustomobject][ordered]@{
    schema_version='mnf-phase08-authorization-receipt/1';release_ref='refs/tags/modules-v0.1.0-r13'
    ...
    historical_r12_sha256=$historyFields.historical_r12_sha256
    historical_history_set_sha256=$historyFields.historical_history_set_sha256
  ...
  $handoffBase = [ordered]@{
    schema_version='mnf-phase08-handoff/1';release_ref='refs/tags/modules-v0.1.0-r13';boundary_sha=('1'*40)
    ...
    attempt_zero_history_path='C:\mnf-phase08-r11\history\attempt-zero.json';attempt_zero_history_sha256=$historyFields.historical_attempt_zero_sha256
    r1_history_path='C:\mnf-phase08-r11\history\r1.json';r1_history_sha256=$historyFields.historical_r1_sha256
    r2_history_path='C:\mnf-phase08-r11\history\r2.json';r2_history_sha256=$historyFields.historical_r2_sha256
    r3_history_path='C:\mnf-phase08-r11\history\r3.json';r3_history_sha256=$historyFields.historical_r3_sha256
    r4_history_path='C:\mnf-phase08-r11\history\r4.json';r4_history_sha256=$historyFields.historical_r4_sha256
    r5_history_path='C:\mnf-phase08-r11\history\r5.json';r5_history_sha256=$historyFields.historical_r5_sha256
    r6_history_path='C:\mnf-phase08-r11\history\r6.json';r6_history_sha256=$historyFields.historical_r6_sha256
    r7_history_path='C:\mnf-phase08-r11\history\r7.json';r7_history_sha256=$historyFields.historical_r7_sha256
    ...
    r12_history_path='C:\\mnf-phase08-r12\\history\\r12.json';r12_history_sha256=$historyFields.historical_r12_sha256
    historical_history_set_sha256=$historyFields.historical_history_set_sha256
...
function Assert-IntentContract {
  ...
  $history = @($policy.initial_attempt_family.terminal_negative_history)
  if ($history.Count -ne 13) { throw 'REL01-HISTORICAL-ATTEMPT: exact attempt-zero through r12 history is required.' }
  $r9 = $history[9]
  if ($r9.attempt -cne 'r9' -or $r9.release_ref -cne 'refs/tags/modules-v0.1.0-r9' -or
      ...
      $r9.tag_object_sha -cne '79d4fa715c6d306e5435d5920c5f92111d5ce13a' -or
      $r9.tag_peeled_source_sha -cne '4158dff7d3b6629861d4f5325573c45f3e3e3436' -or
      $r9.hosted_run_present -ne $false -or $null -ne $r9.run_id -or $null -ne $r9.run_attempt -or
      $r9.failure_stage -cne 'pre_locator_strict_mode_history_schema_failure' -or
      $r9.failure_code -cne 'P08-PREPARE-HISTORY-SCHEMA' -or
      $r9.locator_count -ne 0 -or $r9.credential_accessed -ne $false -or
      $r9.authorization_packet_count -ne 0 -or $r9.authorization_receipt_count -ne 0 -or
      $r9.handoff_count -ne 0 -or $r9.publish_one_count -ne 0 -or $r9.mutation_count -ne 0 -or
      $r9.successor_count -ne 0 -or $r9.record_sha256 -cne (Get-IntentHistoryRecordSha256 -Record $r9)) {
    throw 'REL01-HISTORICAL-R9: protected r9 pre-locator StrictMode terminal evidence drifted.'
  }
  $r10 = $history[10]
  if ($r10.attempt -cne 'r10' -or $r10.release_ref -cne 'refs/tags/modules-v0.1.0-r10' -or
      $r10.source_sha -cne 'd49edc53fb4ffca375e562a23789fb76bf8c41e2' -or
      $r10.tag_object_sha -cne '0546025c61d08a0973a2bb6040cbb19104ae64d1' -or
      $r10.tag_peeled_source_sha -cne 'd49edc53fb4ffca375e562a23789fb76bf8c41e2' -or
      $r10.clean_clone_tag_verified -ne $true -or $r10.hosted_run_present -ne $false -or
      $null -ne $r10.run_id -or $null -ne $r10.run_attempt -or $r10.prepare_attempt_completed -ne $false -or
      $r10.failure_stage -cne 'prepare_attempt_initial_release_ref_binding' -or $r10.failure_code -cne 'REL01-REF' -or
      $r10.prepared_bundle_count -ne 0 -or $r10.publisher_dry_run_count -ne 0 -or $r10.observation_count -ne 0 -or
      $r10.authorization_packet_count -ne 0 -or $r10.authorization_receipt_count -ne 0 -or $r10.handoff_count -ne 0 -or
      $r10.publish_one_count -ne 0 -or $r10.registry_operation_count -ne 0 -or $r10.credential_accessed -ne $false -or
      $r10.publication_performed -ne $false -or $r10.mutation_count -ne 0 -or $r10.successor_count -ne 0 -or
      $r10.authority_acquired -ne $false -or $r10.reason -cne 'terminal_clean_clone_initial_release_ref_binding_failure' -or
  ...
  $r12 = $history[12]
  if ($r12.attempt -cne 'r12' -or $r12.release_ref -cne 'refs/tags/modules-v0.1.0-r12' -or
      $r12.source_sha -cne '5e7b19cdc74ec11d5c524ff34a36c266b15bba39' -or
      $r12.tag_object_sha -cne '57b76c9f9044d3190acc1e4c3fb7ada516f4dece' -or
      $r12.failure_code -cne 'REL01-REF' -or $r12.authority_acquired -ne $false -or
      $r12.publication_performed -ne $false -or $r12.mutation_count -ne 0 -or
      $r12.record_sha256 -cne (Get-IntentHistoryRecordSha256 -Record $r12)) {
    throw 'REL01-HISTORICAL-R12: r12 must remain immutable REL01-REF terminal evidence only.'
  }
  if ($policy.initial_attempt_family.current_attempt -cne 'r13' -or
      $policy.initial_profile.release_ref -cne 'refs/tags/modules-v0.1.0-r13' -or
      $policy.initial_attempt_family.history_set_sha256 -cne (Get-IntentHistorySetSha256 -History $history)) {
    throw 'REL01-R13-BINDING: r13 must bind all thirteen terminal histories.'
  }
  ...
  if (($history.attempt -join ',') -cne 'attempt_zero,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12') { throw 'REL01-HISTORY-ORDER: terminal history order drifted.' }
  ...
  if (@($history.record_sha256 | Select-Object -Unique).Count -ne 13) { throw 'REL01-HISTORY-DIGEST: terminal history digests are not distinct.' }
  ...
  if ($policy.initial_attempt_family.current_attempt -cne 'r13' -or
      $policy.initial_profile.release_ref -cne 'refs/tags/modules-v0.1.0-r13' -or $policy.initial_profile.correction_sequence -ne 0 -or
  ...
  if ($schema.'$defs'.initialIntent.properties.release_ref.const -cne 'refs/tags/modules-v0.1.0-r13') {
    throw 'REL01-INITIAL-PROFILE: initial schema does not require r12.'
  }
  if ($preparedSchema.properties.release_ref.pattern -cne '^refs/tags/modules-(v0[.]1[.]0-r13|correction-[1-9][0-9]*)$') {
