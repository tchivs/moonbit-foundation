...
function Assert-InitialAttemptFamily {
  ...
  if ($history.Count -ne 7 -or ($history.attempt -join ',') -cne 'attempt_zero,r1,r2,r3,r4,r5,r6') {
    Throw-ReleaseRule -Id 'REL01-HISTORY-ORDER' -Message 'attempt-zero, r1, r2, r3, r4, r5, and r6 are required in canonical order.'
  }
  ...
  if (@($history.record_sha256 | Select-Object -Unique).Count -ne 7) {
  ...
  if ($Control.initial_attempt_family.history_set_profile -cne 'sha256-of-lf-joined-record-sha256-in-canonical-attempt-order' -or
      $Control.initial_attempt_family.history_set_sha256 -cne $setDigest -or $Control.initial_attempt_family.current_attempt -cne 'r7') {
...
if ($IntentKind -ceq 'initial') {
  ...
  if (@($control.initial_attempt_family.terminal_negative_history.source_sha) -ccontains $SourceSha) { Throw-ReleaseRule -Id 'REL01-HISTORICAL-SOURCE' -Message 'a terminal-negative source cannot be reused as r7 current authority.' }
  ...
  if (-not [string]::IsNullOrEmpty($PredecessorSourceSha) -or $PredecessorSequence -ne -1 -or
    ...
    Throw-ReleaseRule -Id 'REL01-CORRECTION-EVIDENCE' -Message 'initial r7 cannot carry correction-lane evidence.'
  }
} else {
