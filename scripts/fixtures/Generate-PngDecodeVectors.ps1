...
$requiredColourSplitGroups = @('colour-srgb-compatible-legacy','colour-gama-retained','colour-chrm-retained','colour-iccp-authoritative','colour-hostile-srgb-length')
...
foreach ($group in $requiredColourSplitGroups) {
  if (@($corpus.cases | Where-Object { $_.PSObject.Properties['comparison_group'] -and $_.comparison_group -ceq $group }).Count -ne 1) { throw "PNG colour split coverage is stale or incomplete: $group" }
}
...
$rows.Add('  id : String')
$rows.Add('  bytes : Bytes')
$rows.Add('  width : UInt64')
$rows.Add('  height : UInt64')
$rows.Add('  channels : UInt64')
$rows.Add('  pixels : Bytes')
$rows.Add('  outcome : String')
$rows.Add('  max_output : UInt64')
$rows.Add('  max_work : UInt64')
$rows.Add('  budget_bytes : UInt64')
$rows.Add('  budget_allocations : UInt64')
$rows.Add('  budget_allocation_size : UInt64')
$rows.Add('  srgb_intent : Int')
$rows.Add('  metadata_key : String')
$rows.Add('  metadata_value : Bytes')
$rows.Add('  icc_profile_length : UInt64')
$rows.Add('  category : String')
$rows.Add('  code : String')
$rows.Add('  context : String')
$rows.Add('  comparison_group : String')
$rows.Add('  baseline : Bool')
$rows.Add('}')
$rows.Add('')

function Split-MoonBytesLiteral([string]$Literal, [int]$Width = 80) {
  if ($null -eq $Literal) { $Literal = '' }
  $inner = $Literal
  ...
  while ($i -lt $inner.Length) {
    ...
    [void]$pieces.Add($inner.Substring($i, $j - $i))
    $i = $j
  }
...
for ($part = 1; $part -le $partCount; $part++) {
  $rows.Add('///|')
  $rows.Add(("fn _generated_png_decode_cases_part{0}() -> Array[PngDecodeVector] {{" -f $part))
  $rows.Add('  [')
  $startIndex = ($part - 1) * $partSize
  $endIndex = [Math]::Min($records.Count, $startIndex + $partSize) - 1
  for ($index = $startIndex; $index -le $endIndex; $index++) {
    $record = $records[$index]
    $case = $record.case
    Assert-Oracle $case
    $png = New-Png $case $record.splits
    $pixels = if ($case.outcome -eq 'accepted') { Moon-Bytes (Hex $case.pixels_hex) } else { '' }
    $channels = if ($case.colour_type -eq 4 -or $case.colour_type -eq 6 -or $null -ne $case.PSObject.Properties['trns_hex']) { 4 } else { 3 }
    $context = if ($null -eq $case.PSObject.Properties['context']) { '' } else { [string]$case.context }
    $maxOutput = if ($null -eq $case.PSObject.Properties['max_output']) { 1024 } else { [uint64]$case.max_output }
    $hasIccp = $null -ne $case.PSObject.Properties['colour_chunks'] -and @($case.colour_chunks | Where-Object { $_.kind -eq 'iCCP' }).Count -gt 0
    $maxWork = if ($null -ne $case.PSObject.Properties['max_work']) { [uint64]$case.max_work } elseif ($hasIccp) { 1048576 } else { 4096 }
    $budgetBytes = if ($null -ne $case.PSObject.Properties['budget_bytes']) { [uint64]$case.budget_bytes } elseif ($hasIccp) { 65536 } else { 4096 }
    $budgetAllocations = if ($null -eq $case.PSObject.Properties['budget_allocations']) { 8 } else { [uint64]$case.budget_allocations }
    $budgetAllocationSize = if ($null -ne $case.PSObject.Properties['budget_allocation_size']) { [uint64]$case.budget_allocation_size } elseif ($hasIccp) { 32768 } else { 1024 }
    $srgbIntent = if ($null -eq $case.PSObject.Properties['srgb_intent']) { -1 } else { [int]$case.srgb_intent }
    $metadataKey = if ($null -eq $case.PSObject.Properties['metadata_key']) { '' } else { [string]$case.metadata_key }
    $metadataValue = if ($null -eq $case.PSObject.Properties['metadata_hex']) { '' } else { Moon-Bytes (Hex $case.metadata_hex) }
    $iccProfileLength = if ($null -eq $case.PSObject.Properties['icc_profile_length']) { 0 } else { [uint64]$case.icc_profile_length }
    $category = if ($null -eq $case.PSObject.Properties['category']) { if ($case.outcome -eq 'limit') { 'resource' } else { 'data' } } else { [string]$case.category }
    $code = if ($null -eq $case.PSObject.Properties['code']) { if ($case.outcome -eq 'limit') { 'budget-exceeded' } else { 'invalid-encoding' } } else { [string]$case.code }
    $baseline = if ($record.baseline) { 'true' } else { 'false' }
    $rows.Add('    PngDecodeVector::{')
    $rows.Add(('      id: "{0}",' -f $record.id))
    Add-MoonBytesField $rows 'bytes' (Moon-Bytes $png)
    $rows.Add(('      width: {0}UL,' -f $case.width))
    $rows.Add(('      height: {0}UL,' -f $case.height))
    $rows.Add(('      channels: {0}UL,' -f $channels))
    Add-MoonBytesField $rows 'pixels' $pixels
    $rows.Add(('      outcome: "{0}",' -f $case.outcome))
    $rows.Add(('      max_output: {0}UL,' -f $maxOutput))
    $rows.Add(('      max_work: {0}UL,' -f $maxWork))
    $rows.Add(('      budget_bytes: {0}UL,' -f $budgetBytes))
    $rows.Add(('      budget_allocations: {0}UL,' -f $budgetAllocations))
    $rows.Add(('      budget_allocation_size: {0}UL,' -f $budgetAllocationSize))
    $rows.Add(('      srgb_intent: {0},' -f $srgbIntent))
    $rows.Add(('      metadata_key: "{0}",' -f $metadataKey))
    Add-MoonBytesField $rows 'metadata_value' $metadataValue
    $rows.Add(('      icc_profile_length: {0}UL,' -f $iccProfileLength))
    $rows.Add(('      category: "{0}",' -f $category))
    $rows.Add(('      code: "{0}",' -f $code))
    $rows.Add(('      context: "{0}",' -f $context))
    $rows.Add(('      comparison_group: "{0}",' -f $record.group))
    $rows.Add(('      baseline: {0},' -f $baseline))
    $rows.Add('    },')
  }
  $rows.Add('  ]')
  $rows.Add('}')
...
$rows.Add('  let cases : Array[PngDecodeVector] = []')
for ($part = 1; $part -le $partCount; $part++) {
  $rows.Add(('  for item in _generated_png_decode_cases_part{0}() {{' -f $part))
  $rows.Add('    cases.push(item)')
  $rows.Add('  }')
}
$rows.Add('  cases')
$rows.Add('}')
