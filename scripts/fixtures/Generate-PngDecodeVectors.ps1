...
$required = @('fixed-rgb-filters-every-idat-byte','dynamic-rgba-filters-semantic-idat-split','hostile-zlib-header','hostile-truncated-deflate','hostile-adler','hostile-filter')
...
$rows.Add('')
$rows.Add('///|')
$rows.Add('priv struct PngDecodeVector { id : String; bytes : Bytes; width : UInt64; height : UInt64; channels : UInt64; pixels : Bytes; outcome : String; context : String }')
$rows.Add('')
$rows.Add('///|')
...
foreach ($case in $corpus.cases) {
  Assert-Oracle $case
  $png = New-Png $case; $pixels = if ($case.outcome -eq 'accepted') { Moon-Bytes (Hex $case.pixels_hex) } else { '' }; $channels = if ($case.colour_type -eq 2) { 3 } else { 4 }; $context = if ($null -eq $case.PSObject.Properties['context']) { '' } else { [string]$case.context }
  $rows.Add(('    PngDecodeVector::{{ id: "{0}", bytes: b"{1}", width: {2}UL, height: {3}UL, channels: {4}UL, pixels: b"{5}", outcome: "{6}", context: "{7}" }},' -f $case.id,(Moon-Bytes $png),$case.width,$case.height,$channels,$pixels,$case.outcome,$context))
}
