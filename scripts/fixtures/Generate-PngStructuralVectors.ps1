...
$OutputPath = Join-Path $Root 'modules\mb-image\png\generated_vectors_wbtest.mbt'
...
$header = @(
  '// Generated by scripts/fixtures/Generate-PngStructuralVectors.ps1. Do not edit.',
  '',
  '///|',
  'priv struct PngGeneratedLimits {',
  '  probe : UInt64',
  '  input : UInt64',
  '  output : UInt64',
  '  width : UInt64',
  ...
  '',
  '///|',
  'priv struct PngGeneratedBudget {',
  '  bytes : UInt64',
  '  allocations : UInt64',
  ...
  '  limits : PngGeneratedLimits',
  '  budget : PngGeneratedBudget',
  '  immutable_caller_state : Bool',
  '}',
  ''
)
function Render-Table([string]$Name) {
  $lines = [Collections.Generic.List[string]]::new()
  $lines.AddRange([string[]]$header)
  $lines.Add('///|')
  $lines.Add("fn $Name() -> Array[PngGeneratedCase] {")
  $lines.Add('  [')
  foreach ($case in $cases.cases) {
    $bytes = Moon-Bytes (Get-CaseBytes $case.construction)
    $limits = @($cases.limits_profiles.($case.limits))
    $budget = @($cases.budget_profiles.($case.budget))
    $preserve = ([bool]$case.options.preserve_opaque_metadata).ToString().ToLowerInvariant()
    $immutable = ([bool]$case.immutable_caller_state).ToString().ToLowerInvariant()
    $lines.Add('    PngGeneratedCase::{')
    $lines.Add(('      id: "{0}",' -f $case.id))
    $lines.Add(('      bytes: b"{0}",' -f $bytes))
    $lines.Add(('      category: "{0}",' -f $case.expected.category))
    $lines.Add(('      code: "{0}",' -f $case.expected.code))
    $lines.Add(('      context: "{0}",' -f $case.expected.context))
    $lines.Add(('      preserve_opaque_metadata: {0},' -f $preserve))
    $lines.Add('      limits: PngGeneratedLimits::{')
    $lines.Add(('        probe: {0}UL,' -f $limits[0]))
    $lines.Add(('        input: {0}UL,' -f $limits[1]))
    $lines.Add(('        output: {0}UL,' -f $limits[2]))
    $lines.Add(('        width: {0}UL,' -f $limits[3]))
    $lines.Add(('        height: {0}UL,' -f $limits[4]))
    $lines.Add(('        pixels: {0}UL,' -f $limits[5]))
    $lines.Add(('        work: {0}UL,' -f $limits[6]))
    $lines.Add('      },')
    $lines.Add('      budget: PngGeneratedBudget::{')
    $lines.Add(('        bytes: {0}UL,' -f $budget[0]))
    $lines.Add(('        allocations: {0}UL,' -f $budget[1]))
    $lines.Add(('        allocation_size: {0}UL,' -f $budget[2]))
    $lines.Add(('        width: {0}UL,' -f $budget[3]))
    $lines.Add(('        height: {0}UL,' -f $budget[4]))
    $lines.Add(('        pixels: {0}UL,' -f $budget[5]))
    $lines.Add(('        depth: {0}UL,' -f $budget[6]))
    $lines.Add(('        work: {0}UL,' -f $budget[7]))
    $lines.Add('      },')
    $lines.Add(('      immutable_caller_state: {0},' -f $immutable))
    $lines.Add('    },')
  }
  $lines.Add('  ]')
  $lines.Add('}')
