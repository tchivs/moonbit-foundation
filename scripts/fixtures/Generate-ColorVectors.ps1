...
function Render-ProfileMoon([object]$Data) {
  ...
  $accepted = @($Data.profile.accepted_tags | ForEach-Object { '"{0}"' -f $_ })
  $lines.Add(('  [{0}]' -f ($accepted -join ', ')))
  $lines.Add('}')
  $lines.Add('')
  $lines.Add('///|')
  $lines.Add('fn generated_rejected_profile_tags() -> Array[String] {')
  $rejected = @($Data.profile.rejected_tags | ForEach-Object { '"{0}"' -f $_ })
  $lines.Add(('  [{0}]' -f ($rejected -join ', ')))
  $lines.Add('}')
  $lines.Add('')
  $lines.Add('///|')
