...
- **D-05:** Retain and validate `initialRandomSeed` instead of accepting and discarding it. Official-spec research must confirm its input type/default and may refine only the checked conversion into the already locked 32-bit PRNG seed, not the algorithm, reset boundary, or output mapping.
...
- **D-07:** Use fixed-capacity operand stack 48 and transient array 32 initialized to zero. Stack underflow, bad integer/index/roll operands, transient out-of-range, invalid subr index, and illegal termination are Data; stack/stem/frame/depth/call/byte/work/geometry ceiling exhaustion is Resource.
- **D-08:** Stem operators validate pairs and clear the stack. `hintmask`/`cntrmask` first consume any stack-resident additional stems, then consume exactly `(stem_count + 7) / 8` raw mask bytes. Hints have no rendering effect and unused trailing mask bits need not be zero. Stem ceiling is 96.
- **D-09:** Resolve subroutine bias solely from each selected INDEX count using `107`, `1131`, or `32768`. Use an explicit iterative call-frame stack with global/local identity and the glyph-selected local environment; never use host recursion. Maximum nested subroutine depth is 10. Root EOF, subr EOF, root `return`, subr `endchar`, out-of-range calls, and trailing bytes after legal termination are Data.
...
- **D-14:** Starting a new moveto implicitly closes the prior contour; legal root `endchar` closes the final contour. Root CharString must terminate exactly once with a clean legal `endchar`; subroutines must terminate with `return`. Type 1 `closepath` is not accepted as a no-op.
