...
Source Sans 3.052R OTF and Source Han Serif CN 2.003R are candidates only. Prior inspection did not finalize the exact selected file, profile, length, hash, CJK FD coverage, or derivative recipe. Plan 107-01 must stop before committing any byte unless the Latin candidate proves static name-keyed CFF1 and the CJK candidate proves static CID-keyed CFF1 with multiple used FDs, FDSelect coverage, distinct local Subrs, and a fixed non-empty high GID. [CITED: https://github.com/adobe-fonts/source-sans/releases/tag/3.052R] [CITED: https://github.com/adobe-fonts/source-han-serif/releases/tag/2.003R] [VERIFIED: 107-CONTEXT D-07/D-08]

**Primary recommendation:** execute exactly three dependent plans: 107-01 freezes fixture/oracle identities, 107-02 freezes public/hostile/glyf facts, and 107-03 consumes those identities in v3 evidence, policy, CI, docs, and the separate native baseline. [VERIFIED: 107-CONTEXT D-23]
...
| Host semantic reader A | Select and pin in 107-01; no floating environment. | Mapping, CFF profile/keying, FD selection, metrics, bounds, and ordered cubic facts. [VERIFIED: 107-CONTEXT D-09] |
| Host semantic reader B | Independently select and pin in 107-01; no shared `mb-font` code. | Cross-check the closed semantic projection from reader A. [VERIFIED: 107-CONTEXT D-09] |
| OTS | Pin source/executable identity in 107-01 if available. | Structural acceptance only; never semantic truth. [VERIFIED: 107-CONTEXT D-09] |

Candidate documentation already identified for reader/tool evaluation: fontTools `TTFont` and recording pens, FreeType character mapping/glyph loading/outline decomposition, and OTS structural sanitization. Exact versions and executable/package digests are deliberately not locked by this research. [CITED: https://fonttools.readthedocs.io/en/latest/ttLib/ttFont.html] [CITED: https://fonttools.readthedocs.io/en/latest/pens/recordingPen.html] [CITED: https://freetype.org/freetype2/docs/reference/ft2-character_mapping.html] [CITED: https://freetype.org/freetype2/docs/reference/ft2-glyph_retrieval.html] [CITED: https://freetype.org/freetype2/docs/reference/ft2-outline_processing.html] [CITED: https://github.com/khaledhosny/ots/blob/v9.2.0/README.md]
...
Host-oracle package/tool selection is intentionally unresolved. Before installing or pinning any candidate, Plan 107-01 must run the correct ecosystem registry check, the GSD package-legitimacy gate where applicable, official-source verification, post-install-script inspection where applicable, and executable/package/archive hashing. A non-OK or irreproducible result blocks intake; it is not a reason to weaken the two-reader/profile gate. [VERIFIED: 107-CONTEXT D-07-D-09] [ASSUMED: exact host package choice]
...
| # | Claim | Risk if wrong |
|---|-------|---------------|
| A1 | fontTools, FreeType, and OTS remain suitable host-tool candidates. [ASSUMED] | Low if 107-01 treats them as candidates and blocks rather than silently falling back. |
| A2 | The current 20-minute CI timeout may need adjustment after licensed CJK coverage is added. [ASSUMED] | Low; the plan retains the timeout until final measurement. |

No exact specimen filename, length, SHA-256, CJK FD count, high GID, subset recipe, reader version, or executable digest is assumed by this research. [VERIFIED: assigned prior-research fact; 107-CONTEXT D-07-D-09]

## Open Questions

1. Which exact official Latin and CJK files pass the complete intake profile and redistribution gate? Resolution: Plan 107-01 evaluates the named releases first and selects only an exact passing artifact. [CITED: https://github.com/adobe-fonts/source-sans/releases/tag/3.052R] [CITED: https://github.com/adobe-fonts/source-han-serif/releases/tag/2.003R]
2. Can two semantically independent readers and OTS be pinned and reconstructed in the controlled host environment? Resolution: provision and hash them in 107-01; absence or disagreement blocks intake. [VERIFIED: 107-CONTEXT D-09]
3. Does the completed v3 lane exceed the current CI timeout? Resolution: measure only after 107-01/02 freeze; increase minimally only from observed duration. [VERIFIED: 107-CONTEXT agent discretion]
