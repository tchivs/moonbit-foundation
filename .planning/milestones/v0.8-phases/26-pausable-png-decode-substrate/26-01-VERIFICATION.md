...
**Re-verification:** Yes — after the metadata-only `PNGS-03` summary correction. A fresh detached-worktree `moon -C modules/mb-image test png --target all --frozen` run passed 84/84 on wasm, wasm-gc, js, and native at `2bd4974`; no implementation changes were introduced.
